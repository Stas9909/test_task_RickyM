...
const config: Config = {
	...
	theme: {
		...
		colors: {
			...
			greenad: 'rgba(var(--color-greenad), <alpha-value>)',
			gray: {
		...
		extend: {
			keyframes: {
				'scale-on':{
					'0%': { opasity: '0', rotate: '0deg', transform: 'translateY(70%) translateX(-50%) scale(0.1)' },
					'100%': { opasity: '1', rotate: '360deg', transform: 'translateY(0) translateX(0) scale(1)' }
				},
				// SLIDE
				'slide-in-right': {
		...
		animation: {
			'scale-on': 'scale-on 5s ease-in-out',
